<?php
		$bno = $_GET['1']; /* bno함수에 idx값을 받아와 넣음*/
		$hit = mysqli_fetch_array(mq("select * from board where idx =1"));
		$hit = $hit['hit'] + 1;
		$fet = mq("update board set hit = '".$hit."' where idx = 1");
		$sql = mq("select * from board where idx=1"); /* 받아온 idx값을 선택 */
