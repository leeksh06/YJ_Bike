...
int cnt = postDAO.getPostCount(userId);
int pageSize = 5;
...
ArrayList<Post> listPost = postDAO.getUserPosts(userId, startRow, pageSize);
%>
<body>
<div id="wrap">
...
<div class="container">
    ...
    <div class="infoContent">
        <form onsubmit="return isAtLeastOneCheckboxChecked();" action="DeletePro.jsp?pageNum=<%=pageNum%>" method="post">
            ...
            <table>
                <h3 style="font-size: 30px;"> 게시글 삭제 </h3>
                <colgroup>
                    <col width="10">
                    <col width="300">
                </colgroup>
                <thead>
                <tr>
                    <th><input type="checkbox" id="cbx_post_chkAll"/></th>
                    <th>내용</th>
                </tr>
                </thead>
                <tbody>
                ...
                %>
                <tr>
                    <td><input type="checkbox" name="chkPost" value="<%=listPost.get(i).getIdx()%>"></td>
                   <td><a href="View4.jsp?idx=<%=listPost.get(i).getIdx()%>"><%=listPost.get(i).getTitle()%></a></td>
            ...
            <div class="page">
                ...
                <span> <a href="MyInfoBoard.jsp?pageNum=<%=startPage-pageBlock%>">이전</a>&nbsp&nbsp</span>
                <% }
                    for (int i = startPage; i <= endPage; i++) { %>
                <span> <a href="MyInfoBoard.jsp?pageNum=<%=i%>"><%=i%></a>&nbsp&nbsp</span>
                <% } %>
                <% if (endPage < pageCount) { %>
                <span> <a href="MyInfoBoard.jsp?pageNum=<%=startPage+pageBlock%>">다음</a></span>
                <% } %>
                <%}%>
            </div>
