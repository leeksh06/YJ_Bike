...
<%
 ...
 boolean isCheckboxChecked = false;
    ...
    <div id="free55">
        <div class="row">
            <table class="table table-striped" style="text-align: center; border: 1px solid #dddddd; width: 100%;border-radius:10px 10px 10px 10px;">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td colspan="2"><%= Post.getTitle().replaceAll(" ", "&nbsp;")
                                .replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
                    </tr>
                    <tr>
                        <td>작성자</td>
                        <td colspan="2"><%= PostDAO.getNickname(Post.getUser_id()) %></td>
                    </tr>
                    <tr>
                        <td>작성일자</td>
                        <td colspan="2"><%=PostDAO.formatDate(Post.getDate()) %></td>
                    </tr>
                    <tr>
                        <td>내용</td>
                        <td colspan="2" style="height: 200px; text-align: left;"><%= Post.getContent().replaceAll(" ", "&nbsp;")
                            .replaceAll("<", "<").replaceAll(">", ">").replaceAll("\n", "<br>") %></td>
                    </tr>
...
<td colspan="2">
    ...
    %>
            ...
            <a href="Untitled-2.jsp" class="btn btn-primary"><button class="jelly">목록</button></a>

			<!-- 해당 글의 작성자가 본인이라면 수정과 삭제가 가능하도록 코드 추가 -->
			<%
    if (userID != null && userID.equals(Post.getUser_id())) {
%>
        <a href="updatewrite.jsp?idx=<%= idx %>" class="btn btn-primary"><button class="jelly">수정</button></a>
<%
    }
...
<form action="replyAction.jsp" method="post" onsubmit="return validateForm();">
    <textarea type="text" class="form-control" placeholder="댓글을 입력하세요" name="content" maxlength="2048" style="width: 99.2%; height: 200px; resize: none;"></textarea>
...
<!-- 댓글 목록 -->
<div class="row">
    <table class="table table-striped" style="text-align: center; width: 100%; height: 200px; border: 1px solid #dddddd">
        <tbody>
            <tr>
...
<% if (userID != null) { %>
    ...
    <% } %>
    <% if (list.get(i).getUser_id().equals(userID) || userID.equals("admin")) { %>
...
<div>
    <%
        if(page2 > 1){
    %>
        <a href="View5.jsp?idx=<%=idx%>&page=<%= page2 - 1 %>"
