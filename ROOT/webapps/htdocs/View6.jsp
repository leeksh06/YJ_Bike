...
<%
 ...
 boolean isCheckboxChecked = false;
    ...
    <h2 class="site_main">나의 작성글</h2><br>
...
<tr>
  <td colspan="2">
    ...
    <%
        ...
        if (Post.getThum() != null && !Post.getThum().isEmpty()) {
			String[] postImages = Post.getThum().split(" ");
            for (int i = 0; i < postImages.length; i++) {
                String imgThum = postImages[i];
    %>
                <img src="/<%=imgThum%>" alt="" style="display: inline-block; width:200px;" />
    <%
            }
        }
    %>
...
//alert("<%=Post.getThum()%>");
            ...
            <a href="Untitled-2.jsp" class="btn btn-primary">목록</a>
			
			<!-- 해당 글의 작성자가 본인이라면 수정과 삭제가 가능하도록 코드 추가 -->
			<%
				if(userID != null && userID.equals(Post.getUser_id())){
			%>
					<a href="updatewrite.jsp?idx=<%= idx %>" class="btn btn-primary">수정</a>
					<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteAction.jsp?idx=<%= idx %>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
<div class="row">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align: center;">
                    <%
                        if(list.get(i).getUser_id() != null && list.get(i).getUser_id().equals(userID)){   //Allow modification and deletion if the user is the author of the reply
                    %>
                   <form name="p_search">
...
<aside id="aside">
    ...
    <h2>Login</h2>
  <form method="post" action="loginAction5.jsp" id="login-form">
